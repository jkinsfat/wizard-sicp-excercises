...
; as usual, but the final parameter's value will be a list of any remaining arguments. For instance, given the definition
; (define (f x y . z) <body>)
...
(define (same-parity . items)
  (define (test-iter test items passed)
    (if (null? items)
      passed
      (test-iter test (cdr items) (if (test (car items)) (cons passed (car items)) passed))))
  (test-iter (if (even? (car items)) even? odd?) items '() ))